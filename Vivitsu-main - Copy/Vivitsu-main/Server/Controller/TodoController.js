...
export const createTodo = async (req, res) => {
  try {
    ...
    try {
      ...
      if (embedding) {
        newTask.embedding = embedding;
      }
    } catch (embError) {
      console.error("Error generating embedding for new task:", embError);
    }
...
export const updateTodo = async (req, res) => {
  try {
    ...
    if (title && updatedTask) {
      try {
        const embedding = await generateEmbedding(title);
        if (embedding) {
          updatedTask.embedding = embedding;
          await updatedTask.save();
        }
      } catch (embError) {
        console.error("Error regenerating embedding for task:", embError);
      }
    }
    if (!updatedTask) {
      return res.status(404).json({ success: false, error: "Task not found" });
