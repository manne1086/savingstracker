...
export const getHardcodedResponse = (query, GUIDED_EXPERIENCES, HARDCODED_RESPONSES) => {
  ...
  // Check for guided experience triggers
  if (normalizedQuery.includes('study plan') ||
    normalizedQuery.includes('learning plan') ||
    normalizedQuery.includes('create study schedule') ||
    normalizedQuery.includes('plan my study') ||
    normalizedQuery.includes('help me plan')
  ) {
  ...
  if (normalizedQuery.includes('quiz me') || normalizedQuery.includes('start quiz') || normalizedQuery.includes('test my knowledge')) {
    return { type: 'guided', experience: 'quiz' };
  }
  ...
  if (normalizedQuery.includes('explain concepts')) {
    return { type: 'hardcoded', response: HARDCODED_RESPONSES["explain concepts"] };
  }
