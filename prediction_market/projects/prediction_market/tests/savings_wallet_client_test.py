...
@pytest.fixture()
def savings_wallet_client(
    algorand_client: AlgorandClient, deployer: SigningAccount
) -> SavingsWalletClient:
    factory = algorand_client.client.get_typed_app_factory(
        SavingsWalletFactory, default_sender=deployer.address
    )
...
def test_says_hello(savings_wallet_client: SavingsWalletClient) -> None:
    result = savings_wallet_client.send.hello(args=("World",))
    assert result.abi_return == "Hello, World"


def test_simulate_says_hello_with_correct_budget_consumed(
    savings_wallet_client: SavingsWalletClient,
) -> None:
    result = (
        savings_wallet_client.new_group()
        .hello(args=("World",))
        .hello(args=("Jane",))
        .simulate()
    )
